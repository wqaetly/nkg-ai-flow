...
describe("runtime / hello-flow end-to-end", () => {
  ...
  it("uses dynamic retry_policy retry-after path inputs", async () => {
    const rt = newRuntime();
    const flow = defineFlow({ id: "retry_policy_dynamic_retry_after_path_e2e", version: "1.0.0", registry: rt.nodeTypeRegistry });
    const start = flow.node("start", { id: "s", position: { x: 0, y: 0 } });
    const source = flow.node("transform", {
      id: "source",
      position: { x: 120, y: 0 },
      config: { value: { code: "payment.rate_limit", retryable: true, meta: { retryAfterMs: 700 } } },
    });
    const retryAfterMsPath = flow.node("transform", {
      id: "retryAfterMsPath",
      position: { x: 120, y: 120 },
      config: { value: "meta.retryAfterMs" },
    });
    const retryAfterAtPath = flow.node("transform", {
      id: "retryAfterAtPath",
      position: { x: 120, y: 240 },
      config: { value: "meta.retryAfterAt" },
    });
    const policy = flow.node("retry_policy", {
      id: "policy",
      position: { x: 360, y: 0 },
    ...
    flow.connect(start.out("out"), source.in("in"));
    flow.connect(start.out("out"), retryAfterMsPath.in("in"));
    flow.connect(start.out("out"), retryAfterAtPath.in("in"));
    flow.connect(source.out("output"), policy.in("error"));
    flow.connect(retryAfterMsPath.out("output"), policy.in("retryAfterMsPath"));
    flow.connect(retryAfterAtPath.out("output"), policy.in("retryAfterAtPath"));
    flow.connect(policy.out("retry"), report.in("in"));
    flow.connect(policy.out("delayMs"), report.in("input"));
    flow.connect(report.out("out"), end.in("in"));
    ...
    const result = await rt.invocationRouter.invoke({
      flowId: "retry_policy_dynamic_retry_after_path_e2e",
      input: null,
    });
  ...
  it("records retry_state failures and schedules the next retry", async () => {
    ...
    expect(retryOutput).toMatchObject({
      ...
      summary: {
        status: "retry",
        mode: "record_failure",
        stateKey: "PAYMENT_RETRY:order-1",
        stateStatus: "waiting",
        attempt: 1,
        nextAttempt: 2,
        maxAttempts: 3,
  ...
  it("persists retry_state unsafe when required idempotency is unknown", async () => {
    ...
    expect(retryOutput).toMatchObject({
      ...
      summary: {
        status: "unsafe",
        mode: "record_failure",
        stateKey: "PAYMENT_RETRY:unsafe-order",
        stateStatus: "unsafe",
        attempt: 1,
        nextAttempt: 1,
        maxAttempts: 3,
  ...
  it("routes drained compensation actions into a rollback branch", async () => {
    ...
    expect(rollbackOutput).toMatchObject({
      ...
      failureRate: 0,
      pendingRate: 1,
      registeredFlowIds: ["rollback_plan_e2e", "rollback_plan_e2e"],
      registeredFlowVersions: ["1.0.0", "1.0.0"],
      registeredRunIds: [result.runRecord.runId, result.runRecord.runId],
      registeredNodeIds: ["second", "first"],
      actions: [
        {
          action: "refund_payment",
          registeredFlowId: "rollback_plan_e2e",
          registeredFlowVersion: "1.0.0",
          registeredRunId: result.runRecord.runId,
          registeredNodeId: "second",
        },
  ...
  it("routes wait_signal to received when an external signal is present", async () => {
    ...
    expect(waitOutput).toMatchObject({
      ...
      summary: {
        ...
        signal: "approved",
        expected: "approved",
        waitFlowId: "wait_signal_received_e2e",
  ...
  it("routes expired wait_signal checkpoints to expired", async () => {
    ...
    expect(waitOutput).toMatchObject({
      ...
      summary: {
        ...
        signal: null,
        expected: "approved",
        waitFlowId: "wait_signal_expired_e2e",
        waitRunId: result.runRecord.runId,
        waitNodeId: "wait",
        requestedAt: expect.any(String),
        expiresAt: expect.any(String),
  ...
  it("invokes a registered child flow with subflow and returns its output", async () => {
    ...
    expect(callOutput).toMatchObject({
      ...
      parentFlowId: "parent_calls_child",
      parentFlowVersion: "1.0.0",
      parentRunId: result.runRecord.runId,
      parentNodeId: "call_child",
  ...
  it("invokes a reusable child flow with subflow_template defaults", async () => {
    ...
    expect(callOutput).toMatchObject({
      ...
      parentFlowId: "parent_calls_template_child",
      parentFlowVersion: "1.0.0",
      parentRunId: result.runRecord.runId,
      parentNodeId: "call_template",
      childStartedAt: expect.any(String),
      childFinishedAt: expect.any(String),
      childDurationMs: expect.any(Number),
      subflowDepth: 0,
  ...
  it("routes subflow_template to missing when the template id is absent", async () => {
    ...
    expect(callOutput).toMatchObject({
      ...
      summary: {
        ...
        flowVersion: "",
        runId: null,
        childTraceId: `${result.runRecord.runId}:call_template:missing_template`,
        parentFlowId: "parent_missing_template",
        parentFlowVersion: "1.0.0",
        parentRunId: result.runRecord.runId,
        parentNodeId: "call_template",
        subflowDepth: 0,
  ...
  it("routes subflow_template input contract failures without invoking the child flow", async () => {
    ...
    expect(callOutput).toMatchObject({
      ...
      summary: {
        ...
        flowVersion: "1.0.0",
        runId: null,
        childTraceId: `${result.runRecord.runId}:call_template:contract_child`,
        parentFlowId: "parent_template_input_contract",
        parentFlowVersion: "1.0.0",
        parentRunId: result.runRecord.runId,
        parentNodeId: "call_template",
        subflowDepth: 0,
  ...
  it("routes subflow_template output contract failures after a successful child run", async () => {
    ...
    expect(callOutput).toMatchObject({
      ...
      summary: expect.objectContaining({
        ...
        flowVersion: "1.0.0",
        childTraceId: `${result.runRecord.runId}:call_template:contract_child`,
        parentFlowId: "parent_template_output_contract",
        parentFlowVersion: "1.0.0",
        parentRunId: result.runRecord.runId,
        parentNodeId: "call_template",
        subflowDepth: 0,
  ...
  it("routes subflow input contract failures without invoking the child flow", async () => {
    ...
    expect(callOutput).toMatchObject({
      ...
      summary: {
        ...
        runId: null,
        childTraceId: `${result.runRecord.runId}:call_child`,
        parentFlowId: "parent_input_contract",
        parentFlowVersion: "1.0.0",
        parentRunId: result.runRecord.runId,
        parentNodeId: "call_child",
        subflowDepth: 0,
  ...
  it("saves a checkpoint with dynamic policy inputs", async () => {
    ...
    expect(checkpointOutput).toMatchObject({
      ...
      checkpointFlowId: "checkpoint_dynamic_policy_e2e",
      checkpointRunId: result.runRecord.runId,
      checkpointNodeId: "checkpoint",
      version: 1,
      ttlMs: 120_000,
  ...
  it("uses dynamic join expectedCount ahead of static config", async () => {
    const rt = newRuntime();
    const flow = defineFlow({ id: "join_dynamic_expected_count_e2e", version: "1.0.0", registry: rt.nodeTypeRegistry });
    const start = flow.node("start", { id: "s", position: { x: 0, y: 0 } });
    const expectedCount = flow.node("transform", {
      id: "expected_count",
      position: { x: 140, y: 0 },
      config: { value: 3 },
    });
    ...
    flow.connect(start.out("out"), expectedCount.in("in"));
    flow.connect(expectedCount.out("out"), upper.in("in"));
    flow.connect(start.out("runInput"), upper.in("input"));
    flow.connect(upper.out("out"), join.in("in"));
    flow.connect(upper.out("output"), join.in("values"));
    flow.connect(expectedCount.out("output"), join.in("expectedCount"));
    flow.connect(join.out("out"), report.in("in"));
    flow.connect(join.out("status"), report.in("input"));
    flow.connect(report.out("out"), end.in("in"));
    ...
    const result = await rt.invocationRouter.invoke({
      flowId: "join_dynamic_expected_count_e2e",
      input: { name: "Flow" },
    });
    const events = await rt.eventBus.store.read(result.runRecord.runId);
    const joinOutput = (
  ...
  it("fans out through three parallel branches and reports branch batches", async () => {
    const rt = newRuntime();
    const flow = defineFlow({ id: "parallel_three_branch_e2e", version: "1.0.0", registry: rt.nodeTypeRegistry });
    const start = flow.node("start", { id: "s", position: { x: 0, y: 120 } });
    const fanout = flow.node("parallel", {
      id: "fanout",
      position: { x: 120, y: 120 },
      config: { branchCount: 3, concurrency: 2 },
    });
    const first = flow.node("transform", {
      id: "first",
      position: { x: 280, y: 20 },
      config: { template: "a:${input.name}" },
    });
    const second = flow.node("transform", {
      id: "second",
      position: { x: 280, y: 120 },
      config: { template: "b:${input.name}" },
    });
    const third = flow.node("transform", {
      id: "third",
      position: { x: 280, y: 220 },
      config: { template: "c:${input.name}" },
    });
    const join = flow.node("join", { id: "join", position: { x: 440, y: 120 } });
    const report = flow.node("transform", {
      id: "report",
    ...
    flow.connect(start.out("out"), fanout.in("in"));
    flow.connect(start.out("runInput"), fanout.in("input"));
    flow.connect(fanout.out("branch1"), first.in("in"));
    flow.connect(fanout.out("branch2"), second.in("in"));
    flow.connect(fanout.out("branch3"), third.in("in"));
    flow.connect(fanout.out("value"), first.in("input"));
    flow.connect(fanout.out("value"), second.in("input"));
    flow.connect(fanout.out("value"), third.in("input"));
    flow.connect(first.out("out"), join.in("in"));
    flow.connect(second.out("out"), join.in("in"));
    flow.connect(third.out("out"), join.in("in"));
    flow.connect(first.out("output"), join.in("values"));
    flow.connect(second.out("output"), join.in("values"));
    flow.connect(third.out("output"), join.in("values"));
    flow.connect(join.out("out"), report.in("in"));
    flow.connect(join.out("values"), report.in("input"));
    flow.connect(report.out("out"), end.in("in"));
    ...
    const result = await rt.invocationRouter.invoke({
      flowId: "parallel_three_branch_e2e",
      input: { name: "Flow" },
    });
    const events = await rt.eventBus.store.read(result.runRecord.runId);
    const fanoutOutput = (
    ...
    expect(result.succeeded).toBe(true);
    expect(result.output).toBe("parallel=a:Flow,b:Flow,c:Flow");
    expect(fanoutOutput).toMatchObject({
      branchCount: 3,
      concurrency: 2,
      ...
      branches: [
        { id: "branch1", index: 0, number: 1, parallelNodeId: "fanout" },
        { id: "branch2", index: 1, number: 2, parallelNodeId: "fanout" },
        { id: "branch3", index: 2, number: 3, parallelNodeId: "fanout" },
      ],
      branchBatches: [
        {
          index: 0,
  ...
  it("uses dynamic slice_items start and end ahead of static config", async () => {
    const rt = newRuntime();
    const flow = defineFlow({ id: "slice_items_dynamic_range_e2e", version: "1.0.0", registry: rt.nodeTypeRegistry });
    const start = flow.node("start", { id: "s", position: { x: 0, y: 0 } });
    const input = flow.node("transform", {
      id: "input",
      position: { x: 120, y: 0 },
    ...
    const startIndex = flow.node("transform", {
      id: "startIndex",
      position: { x: 120, y: 120 },
      config: { value: 2 },
    });
    const endIndex = flow.node("transform", {
      id: "endIndex",
      position: { x: 120, y: 240 },
      config: { value: 4 },
    });
    ...
    flow.connect(start.out("out"), input.in("in"));
    flow.connect(start.out("out"), startIndex.in("in"));
    flow.connect(start.out("out"), endIndex.in("in"));
    flow.connect(input.out("out"), slice.in("in"));
    flow.connect(input.out("output"), slice.in("items"));
    flow.connect(startIndex.out("output"), slice.in("start"));
    flow.connect(endIndex.out("output"), slice.in("end"));
    flow.connect(slice.out("out"), map.in("in"));
    flow.connect(slice.out("items"), map.in("items"));
    flow.connect(map.out("out"), report.in("in"));
    flow.connect(map.out("items"), report.in("input"));
    ...
    const result = await rt.invocationRouter.invoke({
      flowId: "slice_items_dynamic_range_e2e",
      input: null,
    });
  ...
  it("uses dynamic slice_items fromEnd and count ahead of static config", async () => {
    const rt = newRuntime();
    const flow = defineFlow({ id: "slice_items_dynamic_from_end_e2e", version: "1.0.0", registry: rt.nodeTypeRegistry });
    const start = flow.node("start", { id: "s", position: { x: 0, y: 0 } });
    const input = flow.node("transform", {
      id: "input",
      position: { x: 120, y: 0 },
    ...
    const startOffset = flow.node("transform", {
      id: "startOffset",
      position: { x: 120, y: 120 },
      config: { value: 1 },
    });
    ...
    const fromEnd = flow.node("transform", {
      id: "fromEnd",
      position: { x: 120, y: 360 },
      config: { value: true },
    });
    const slice = flow.node("slice_items", {
      id: "slice",
      position: { x: 320, y: 0 },
      config: { start: 0, count: 2, fromEnd: false },
    });
    const map = flow.node("map_items", {
      id: "map",
      position: { x: 480, y: 0 },
    ...
    flow.connect(start.out("out"), input.in("in"));
    flow.connect(start.out("out"), startOffset.in("in"));
    flow.connect(start.out("out"), count.in("in"));
    flow.connect(start.out("out"), fromEnd.in("in"));
    flow.connect(input.out("out"), slice.in("in"));
    flow.connect(input.out("output"), slice.in("items"));
    flow.connect(startOffset.out("output"), slice.in("start"));
    flow.connect(count.out("output"), slice.in("count"));
    flow.connect(fromEnd.out("output"), slice.in("fromEnd"));
    flow.connect(slice.out("out"), map.in("in"));
    flow.connect(slice.out("items"), map.in("items"));
    flow.connect(map.out("out"), report.in("in"));
    flow.connect(map.out("items"), report.in("input"));
    ...
    const result = await rt.invocationRouter.invoke({
      flowId: "slice_items_dynamic_from_end_e2e",
      input: null,
    });
  ...
  it("uses dynamic batch_items policy inputs", async () => {
    const rt = newRuntime();
    const flow = defineFlow({ id: "batch_items_dynamic_policy_e2e", version: "1.0.0", registry: rt.nodeTypeRegistry });
    const start = flow.node("start", { id: "s", position: { x: 0, y: 0 } });
    const input = flow.node("transform", {
      id: "input",
      position: { x: 120, y: 0 },
    ...
    flow.connect(input.out("out"), batch.in("in"));
    flow.connect(input.out("output"), batch.in("items"));
    flow.connect(size.out("output"), batch.in("size"));
    flow.connect(includePartial.out("output"), batch.in("includePartial"));
    flow.connect(batch.out("out"), report.in("in"));
    flow.connect(batch.out("count"), report.in("input"));
    flow.connect(report.out("out"), end.in("in"));
    ...
    const result = await rt.invocationRouter.invoke({
      flowId: "batch_items_dynamic_policy_e2e",
      input: null,
    });
  ...
  it("skips the current for item when loop_continue runs inside the body", async () => {
    const rt = newRuntime();
    const flow = defineFlow({
      id: "for_continue_e2e",
      version: "1.0.0",
    ...
    flow.connect(start.out("out"), begin.in("in"));
    flow.connect(begin.out("body"), shouldSkip.in("in"));
    flow.connect(begin.out("index"), shouldSkip.in("input"));
    flow.connect(begin.out("iterationId"), continuer.in("iterationId"));
    flow.connect(begin.out("iterationKey"), continuer.in("iterationKey"));
    flow.connect(begin.out("iterationSequence"), continuer.in("iterationSequence"));
    flow.connect(shouldSkip.out("true"), continuer.in("in"));
    flow.connect(shouldSkip.out("false"), emit.in("in"));
    flow.connect(begin.out("index"), emit.in("input"));
    flow.connect(emit.out("out"), loopEnd.in("body_done"));
    flow.connect(emit.out("output"), loopEnd.in("result"));
    flow.connect(loopEnd.out("done"), report.in("in"));
    flow.connect(loopEnd.out("results"), report.in("input"));
    flow.connect(report.out("out"), exit.in("in"));
    ...
    expect(result.succeeded).toBe(true);
    expect(result.output).toBe("results=i=0,i=1,i=3");
    expect(continueOutput).toMatchObject({
      status: "continue",
      reason: "skip_two",
      iterationId: "begin:2",
    ...
    expect(loopOutput).toMatchObject({
      status: "done",
      iterationCount: 4,
      iterationIds: ["begin:0", "begin:1", "begin:2", "begin:3"],
      iterationKeys: [
        "for_begin:begin:0",
        "for_begin:begin:1",
        "for_begin:begin:2",
      ...
      iterationSequences: [0, 1, 2, 3],
    });
    expect(progress[5]?.payload).toMatchObject({
      iteration: 2,
      status: "continue",
      controlReason: "skip_two",
    });
  });

  it("stops a loop_begin block when loop_break runs inside the body", async () => {
    const stepNode = defineNode({
      type: "loop_break_step",
      typeVersion: "1.0.0",
      title: "Loop Break Step",
      ports: [
        { id: "state", direction: "input", kind: "data", label: "State" },
        { id: "nextState", direction: "output", kind: "data", label: "Next State" },
      ],
    ...
    const begin = flow.node("loop_begin", {
      id: "begin",
      position: { x: 260, y: 0 },
      config: { checkMode: "after", maxIterations: 5 },
    });
    const step = flow.node("loop_break_step", {
      id: "step",
      position: { x: 400, y: 0 },
    });
    const shouldBreak = flow.node("condition", {
      id: "should_break",
      position: { x: 540, y: 0 },
      config: { expression: "input.step == 2" },
    });
    const breaker = flow.node("loop_break", {
      id: "break",
      position: { x: 680, y: -80 },
      config: { reason: "stop_at_second_step" },
    });
    const loopEnd = flow.node("loop_end", {
      id: "loop_end",
      position: { x: 820, y: 0 },
    ...
    flow.connect(begin.out("iterationKey"), breaker.in("iterationKey"));
    flow.connect(begin.out("iterationSequence"), breaker.in("iterationSequence"));
    flow.connect(step.out("out"), shouldBreak.in("in"));
    flow.connect(step.out("nextState"), shouldBreak.in("input"));
    flow.connect(step.out("nextState"), loopEnd.in("nextState"));
    flow.connect(shouldBreak.out("true"), breaker.in("in"));
    flow.connect(shouldBreak.out("false"), loopEnd.in("body_done"));
    flow.connect(loopEnd.out("done"), report.in("in"));
    flow.connect(loopEnd.out("controlReason"), report.in("input"));
    flow.connect(report.out("out"), exit.in("in"));

    await registerAndPromote(rt, flow);

    const result = await rt.invocationRouter.invoke({
      flowId: "loop_break_e2e",
      input: null,
    });
    const events = await rt.eventBus.store.read(result.runRecord.runId);
    const breakOutput = (
    ...
    expect(result.succeeded).toBe(true);
    expect(result.output).toBe("break=stop_at_second_step");
    expect(breakOutput).toMatchObject({
      status: "break",
      reason: "stop_at_second_step",
      iterationId: "begin:1",
      iterationKey: "loop_begin:begin:1",
      iterationSequence: 1,
      summary: {
        status: "break",
        reason: "stop_at_second_step",
    ...
    expect(progress.at(-1)?.payload).toMatchObject({
      iteration: 1,
      status: "break",
      controlReason: "stop_at_second_step",
    });
    expect(
      events.filter((event) => event.kind === "node_started" && event.nodeId === "step"),
    ).toHaveLength(2);
  });
  ...
  it("uses dynamic for_begin range inputs when executing the loop block", async () => {
    const rt = newRuntime();
    const flow = defineFlow({
      id: "for_block_dynamic_range_e2e",
      version: "1.0.0",
      registry: rt.nodeTypeRegistry,
    });
    const start = flow.node("start", { id: "s", position: { x: 0, y: 0 } });
    const startValue = flow.node("transform", {
      id: "startValue",
      position: { x: 120, y: 0 },
      config: { value: 2 },
    });
    const endValue = flow.node("transform", {
      id: "endValue",
      position: { x: 120, y: 120 },
      config: { value: 8 },
    });
    const stepValue = flow.node("transform", {
      id: "stepValue",
      position: { x: 120, y: 240 },
      config: { value: 3 },
    });
    ...
    const exit = flow.node("end", { id: "e", position: { x: 940, y: 0 } });

    flow.connect(start.out("out"), startValue.in("in"));
    flow.connect(startValue.out("out"), endValue.in("in"));
    flow.connect(endValue.out("out"), stepValue.in("in"));
    flow.connect(stepValue.out("out"), begin.in("in"));
    flow.connect(startValue.out("output"), begin.in("start"));
    flow.connect(endValue.out("output"), begin.in("end"));
    flow.connect(stepValue.out("output"), begin.in("step"));
    flow.connect(begin.out("body"), body.in("in"));
    flow.connect(begin.out("index"), body.in("input"));
    flow.connect(body.out("out"), loopEnd.in("body_done"));
    flow.connect(body.out("output"), loopEnd.in("result"));
    ...
    const result = await rt.invocationRouter.invoke({
      flowId: "for_block_dynamic_range_e2e",
      input: null,
    });

    expect(result.succeeded).toBe(true);
    expect(result.output).toBe("results=i=2,i=5");
