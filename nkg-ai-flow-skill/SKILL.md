...
- **环境变量读取必须走 `VariableStore`**:任何节点、runner、smoke、CLI 都禁止直接读 `process.env`、禁止自己 `dotenv.config()`、禁止在 app/scripts 目录新建 `.env*` 文件冒充配置源。
...
- 对 `src/agent-flow/hex-advisor.flow.json`,使用伴生文件:`*.flow.env.json`(可提交的默认值或非敏感占位)和 `*.flow.local.env.json`(本机真实配置,**必须** gitignore)。
- Builder 和 Flow JSON 用 `$var.NAME` 引用,不硬编码 key/URL/model。
- 运行时入口、smoke、CLI 通过 `createFlowScopedStores({ flowPath })` 读伴生文件,再调 `bootstrapDefaults({ variables, variableNames })`。
- `.env.example` 仅作文档,不是运行时输入源。
- 缺失值、占位值、伴生 JSON 非法都必须硬失败。**禁止** mock provider、空字符串默认、`.env.local` 兜底、test-only bypass。

### 严禁绕开项目环境变量模块

读配置**必须**走 `VariableStore`(节点里 `ctx.variables.read("LLM_API_KEY")` / `ctx.variables.read("LLM_BASE_URL")`),**禁止**任何节点、runner、smoke、CLI 自己造一套进程级 env 读取。下面这些做法全是违规,无论是不是"只在测试里用一下":
...
- ❌ 在 app / scripts / 节点目录里新建 `.env` / `.env.local` / `.env.dev` 当配置源(本机配置请放 `*.flow.local.env.json`)。
- ❌ 在 builder/runner 写 `process.env.X ?? defaultValue`、`process.env.X || "fallback"`——绕过 `VariableStore` 的 trace 与可枚举性,无法在 Studio / Run Event 里追踪到底用了哪条值。
- ❌ 测试里用 `NODE_ENV === "test" ? mockKey : realKey` 这种分支选凭据。

进程环境变量只能作为启动期的输入源喂给 `bootstrapDefaults`,进入运行时后所有读取一律通过 `VariableStore`。开发、smoke、CI、生产全都一样。`VariableStore` 可枚举可 trace,`process.env` 是黑盒。
...
- [ ] **配置读取都走 `VariableStore`**;PR diff 里 grep `process.env` / `dotenv` / `\.env\.local` 必须只命中 `bootstrapDefaults` 入口和 `.gitignore`。
